...
Public Class FReceipt
    ...
    Private Sub LoadCustomer()
        Try
            Call buka()
            sql = "SELECT CustName FROM Customer As C, POS as P WHERE C.CustomerNo = P.CustomerNo AND P.InvoiceNo = '" & FPOS.lblInvoiceNo.Text & "'"
    ...
    Private Sub getCashChange()
        Try
            Call buka()
            sql = "SELECT * FROM bayar WHERE InvoiceNo = '" & FPOS.lblInvoiceNo.Text & "'"
            cmd = New SqlCommand(sql, koneksi)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)

            If dr.Read = True Then
                lblCash.Text = Format(dr("Cash"), "0")
                lblChange.Text = Format(dr("Kembalian"), "0")
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
        Finally
    ...
    Private Sub getEmployee()
        Try
            Call buka()
            sql = "SELECT S.StaffID, akhir, awal FROM STaff as S, POS as P WHERE S.StaffID = P.StaffID AND InvoiceNo = '" & FPOS.lblInvoiceNo.Text & "'"
            cmd = New SqlCommand(sql, koneksi)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)

            If dr.Read = True Then
                lblEmpName.Text = dr("awal") & " " & dr("akhir")
            End If
        Catch ex As Exception
            MsgBox(ex.Message)
        Finally
    ...
    Private Sub LoadReceipt()
        ...
        Try
            Call buka()
            sql = "SELECT INama, itemprice, [Quantity], POSDATE, (itemPrice * [Quantity]) as TotalPrice FROM [barang] as I, POSDetail as PD, POS as P WHERE I.ItemNo = PD.itemNo And PD.InvoiceNO = P.InvoiceNo AND P.InvoiceNo = '" & FPOS.lblInvoiceNo.Text & "' ORDER By INama"
            cmd = New SqlCommand(sql, koneksi)
            dr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            dgw.Rows.Clear()
            getTotal = 0.0
            ...
            Do While dr.Read = True
                dgw.Rows.Add(dr("Quantity"), dr("INama"), dr("Itemprice"), dr("TotalPrice"))
            ...
            lblTotal.Text = Format(getTotal, "0")
    ...
    Private Sub frmPrintReceipt_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        lblInvoice.Text = FPOS.lblInvoiceNo.Text
        LoadCustomer()
        'GetVatNonVat()
        getCashChange()
        getEmployee()
